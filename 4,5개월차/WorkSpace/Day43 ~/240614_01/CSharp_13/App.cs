...
namespace CSharp_13
{
    ...
    {
        // 클래스는 변수초기화를 선언과 동시에 할 수 있다.
        private int _number = 100;

        // Only 프로퍼티(변수 초기화 불가능)
        public int Number
        {
            get
            {
        ...
        // 변수이면서 프로퍼티(변수초기화가 가능)
        public int Money { get; private set; } = 20; // 이 경우엔 get은 어디서든 가능, set은 내꺼(지금은 MyClass)에서만 가능
        // 웬만해선 set은 막아두긴 한다.
        // 그리고 선언과 동시에 초기화
    ...
    // Actor를 부모 class 로 하는 이유는 아키텍처를 같이 쓰기 위해서 그리고
    // Actor들을 한 번에 관리하기 위해서, (씬에서 vector<Actor*> 로 들고 컨트롤하기 위해서)

    // SpriteActor
    //  => 스프라이트를 출력하는 함수'기능'을 같이 쓰기위해서
    ...
    // 인터페이스는 '선언만' 가능하고 '함수만' 선언 가능하다
    //  + 프로퍼티는 함수다. 프로퍼티도 선언 가능
    // 정의는 불가능
